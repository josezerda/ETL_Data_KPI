...
# Primera corrida: traer desde 2025-01-01 hasta 2025-02-01 (incluyendo 2025-02-01)
BOOTSTRAP_START = "2025-01-01"
BOOTSTRAP_LAST_DAY = "2025-02-01"
BOOTSTRAP_END_EXCL = "2025-02-02"  # end exclusivo para incluir 2025-02-01 completo
...
def _as_dict(obj) -> dict:
    """Convierte el objeto agg a dict de forma robusta."""
...
def _read_csv_header(path: Path) -> list[str] | None:
    """Lee el header del CSV si existe y tiene contenido."""
...
def _write_new_csv(client: RESTClient, symbol: str, start: str, end_excl: str) -> int:
    """
    Crea/reescribe el CSV completo con el rango [start, end_excl).
    Devuelve cantidad de filas escritas.
    """
    OUT_FILE.parent.mkdir(parents=True, exist_ok=True)

    it = client.list_aggs(symbol, 1, "minute", start, end_excl, limit=50000)
    first = next(iter(it), None)

    # Reescribe el archivo aunque venga vacío
    with OUT_FILE.open("w", newline="", encoding="utf-8") as f:
        if first is None:
            # CSV vacío (sin header) para dejar evidencia
        ...
        for a in client.list_aggs(symbol, 1, "minute", start, end_excl, limit=50000):
...
def _append_day_to_csv(client: RESTClient, symbol: str, day: str) -> int:
    """
    Appendea al CSV el día completo 'day' usando rango [day, day+1).
    ...
    # Si el archivo no existe o está vacío, lo creamos con header
    if header is None:
        with OUT_FILE.open("w", newline="", encoding="utf-8") as f:
            fieldnames = list(row1.keys())
            writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction="ignore")
    ...
    # Si ya existe, appendeamos respetando header existente
...
def massive_daily():
    ...
    def fetch_nvda_aggs_to_one_csv():
        ...
        # 1) Primera corrida: un solo CSV con todo el rango bootstrap
        ...
        # 2) Corridas siguientes: agregar 1 día al mismo CSV
